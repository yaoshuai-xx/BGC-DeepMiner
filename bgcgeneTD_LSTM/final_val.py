...
class final_validation:
    def __init__(self, x_true, x_pred, y_true, y_pred, outpath) -> None:
        self.x_true = x_true
        self.x_pred = x_pred
        self.y_true = y_true
        self.y_pred = y_pred
    ...
    def table(self, x_true, x_pred, label):
        ...
        # 保存图片
        file_path = self.outpath + f'_{label}_cm.svg'
    ...
    def plot_step(self, x_true, x_pred, x):
        ...
        file_path = self.outpath + f'_{x}_ROC.svg'
    ...
    def plot_two(self):
        ...
        tpr = dict()
        roc_auc = dict()
        fpr['bgc'], tpr['bgc'], _ = roc_curve(self.x_true.astype('int'), self.x_pred)
        fpr['gene'], tpr['gene'], _ = roc_curve(self.y_true.astype('int'), self.y_pred)
        # 计算AUC
        ...
        plt.legend(loc="lower right")
        file_path = self.outpath + f'_BgcandGene_ROC.svg'
        plt.savefig(file_path, dpi=300)
        plt.close() 
    ...
    def result(self):
        ...
        self.plot_step(self.y_true,self.y_pred,'Gene')
        self.table(self.y_true,self.y_pred,'Gene')
        self.plot_two()

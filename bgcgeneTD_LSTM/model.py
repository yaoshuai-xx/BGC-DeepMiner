...
class LSTMNET(pt.nn.Module):
    def __init__(self, embedding_dim, hidden_dim, num_layers, dropout=0.5, max_len=128):
        ...
        self.TD = TimeDistributed(FeatLayer(32*hidden_dim*2, max_len), batch_first=True)
...
class FeatLayer(pt.nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        self.feat = pt.nn.Sequential(
                ...
                *[pt.nn.Sequential(pt.nn.Linear(sizes[i], sizes[i+1]), pt.nn.Sigmoid()) for i in range(len(sizes)-1)],
                pt.nn.Linear(sizes[-1],dim_out), #(batch_size, max_len)
                pt.nn.Sigmoid()
                )
...
class TimeDistributed(pt.nn.Module):
    ...
    def forward(self, x):
            ...
            # 输入(batch_size, max_len*hidden_dim*2)
            y = self.module(x_reshaped)
            # 输出(batch_size, max_len)
            ## 还原原始形状
